...
include_once "../includes/conexaoPdo.php";
$pdo = conexao();
// Função tempo
...
// Verifica se existe usuário logado a mais de 1h30.
//Data de hoje
$horaFim = date("Y-m-d H:i:s");

$sql = $pdo->query("SELECT * FROM loja_carrinho "); 
$row = $sql->rowCount();
...
if ($row > 0){
    ...
    while ($mostrar = $sql->fetch(PDO::FETCH_ASSOC)){ 
        ...
        $id=$mostrar['id'];
        $horaInicio=$mostrar['data'];

        $resultado = datediff('n', $horaInicio, $horaFim, false);

        //Verifica se esta a mais de 1h30 offline
        if ($resultado >= 1440) {
            
            //Se sim, então atualiza para offline
            // Deleta Produtos
			$pdo->query("DELETE FROM loja_carrinho WHERE id='$id'");
        }
    }
}
?>