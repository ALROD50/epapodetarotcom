...
$id = @$_GET['id'];
//Inclui o filtro
include "atendimento_filtro.php";

// soma minutos atendidos 
$sql4 = $pdo->query("SELECT SUM(duracao) as soma4 FROM atendimento WHERE data BETWEEN '$ano_escolhido-$tab-01' AND '$ano_escolhido-$tab-31 23:59:59' AND id_tarologo='$id' ");
...
?>
<span style="font-size:15px; color:#383C3F;">Minutos total atendidos neste mês: </span><strong><span class="label label-success"><?php echo $valor44; ?></span></strong>
...
<?php
// Comissão 1 ao 14
$sql4 = $pdo->query("SELECT SUM(duracao) as soma5 FROM atendimento WHERE data BETWEEN '$ano_escolhido-$tab-01' AND '$ano_escolhido-$tab-14 23:59:59' AND id_tarologo='$id' ");
$cont4 = $sql4->fetch(PDO::FETCH_ASSOC);
...
<?php
// Comissão 15 ao 31
$sql4 = $pdo->query("SELECT SUM(duracao) as soma6 FROM atendimento WHERE data BETWEEN '$ano_escolhido-$tab-15' AND '$ano_escolhido-$tab-31 23:59:59' AND id_tarologo='$id' ");
$cont4 = $sql4->fetch(PDO::FETCH_ASSOC);
...
<?php
$sql = $pdo->query("SELECT * FROM atendimento WHERE data BETWEEN '$ano_escolhido-$tab-01' AND '$ano_escolhido-$tab-31 23:59:00' AND id_tarologo='$id' ORDER BY id DESC ");
$row = $sql->rowCount();
if ($row > 0){
?>
